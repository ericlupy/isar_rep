...
from uuv_baselines.uuv_gym import UUVEnv, EarlyStoppingCallback
...
working_dir = 'uuv_baselines_stlgym_' + args.algo
...
env = UUVEnv(initial_states=bad_states)
...
class UUVCustomActorCriticPolicy(ActorCriticPolicy):
...
class UUVCustomSACPolicy(SACPolicy):
...
if args.algo == 'ppo':
    model = PPO(UUVCustomActorCriticPolicy, env, verbose=1)
elif args.algo == 'sac':
    model = SAC(UUVCustomSACPolicy, env, verbose=1)
elif args.algo == 'a2c':
    model = A2C(UUVCustomActorCriticPolicy, env, verbose=1)
else:
    raise NotImplementedError
