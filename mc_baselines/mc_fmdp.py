...
from mc_baselines.mc_gym import MCEnvFMDP, EarlyStoppingCallback
...
parser.add_argument("--steps", help="training total time steps, default 100000", default=100000)
...
working_dir = 'mc_baselines_fmdp_' + args.algo
...
env = MCEnvFMDP(initial_states=bad_states)
...
class MCCustomActorCriticPolicyAug(ActorCriticPolicy):
...
class MCCustomSACPolicyAug(SACPolicy):
...
if args.algo == 'ppo':
    model = PPO(MCCustomActorCriticPolicyAug, env, verbose=1)
elif args.algo == 'a2c':
    model = A2C(MCCustomActorCriticPolicyAug, env, verbose=1)
elif args.algo == 'sac':
    model = SAC(MCCustomSACPolicyAug, env, verbose=1)
else:
    raise NotImplementedError
